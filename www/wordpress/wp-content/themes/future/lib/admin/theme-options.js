...
(function($){
	...
	/** Options Tabs */
	function futureOptionsTabs() {
		
		var relid = $.cookie( 'future_tab_relid' );
		
		if( relid >= 1  ) {
			futureOptionsTabControl( relid );
		} else {
			futureOptionsTabControl( 0 );
		}
		
		$( '.future-group-tab-link-a' ).click( function() {
			
			relid = $(this).attr( 'data-rel' );
			$.cookie( 'future_tab_relid', relid );
			futureOptionsTabControl( relid );		
	...
	function futureOptionsTabControl( relid ) {
		
		$( '.future-group-tab' ).each( function() {
		...
		$( '.future-group-tab-link-li' ).each( function() {
