...
jQuery( document ).on('load ready', function() {
	...
	jQuery('#tour_btn').on('click', function() {
		...
		//RATE OPTIMIZER
		jQuery('#customize-theme-controls>ul').after('<div class="rate_optimizer"><h3><i class="fa fa-start"></i> Rate Optimizer</h3><p>'+objectL10n.rate_opt+' <a href="https://wordpress.org/support/view/theme-reviews/optimizer?filter=5" targe="_blank">Rate Optimizer</a>? <img title="Help Us" src="'+objectL10n.rate_smile+'" /></p></div>');
		jQuery('.rate_optimizer h3').toggle(function(){ 
			jQuery('.rate_optimizer p').slideDown(200);
		},function(){
			jQuery('.rate_optimizer p').slideUp(200);
		 });
